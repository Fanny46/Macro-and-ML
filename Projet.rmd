...
# Calcul des lags et différences
df <- df %>%
  arrange(row_number()) %>%
  mutate(
...
# cible = inflation future
df_ML <- df
df_ML$y_lead <- dplyr::lead(df_ML$infl, h)
...
all_vars <- setdiff(names(df_ML), c("y_lead", "infl"))
for (var in all_vars) {
  for (lag in 1:p) {
...
infl_pred <- predict(rf_model, X_test)

# Reconstruction du CPI
CPI_last <- df$CPIAUCSL[
  train_size + p + h
]

CPI_pred <- CPI_last * exp(cumsum(infl_pred) / 100)

CPI_true <- df$CPIAUCSL[(train_size + p + h + 1):nrow(df)]

## évaluation des performances en comparant y_pred et y_test
rmse <- sqrt(mean((CPI_true - CPI_pred)^2))
rmse # je trouve 45, cela me parait élevé

r2 <- cor(CPI_true, CPI_pred)^2
